...
public class TurretDummySimulator implements DummySimulationGenerator {
	...
	public SimulationResult generateDummySimulation() {
		...
		for (int i = 0; i < simulationLength; i++) {
			TowerState thisState = new TowerState(X, Y, orientationPerStep * i, 0);
			result.add(new SimulationState(Collections.<Integer,ProjectileState>emptyMap(),
										   Collections.singletonMap(0, thisState)));
		}
